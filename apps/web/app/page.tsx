...
function MediaPanel({ csrf, role, activeChannelId, onOpenPage }: { csrf: string; role: Role; activeChannelId: string; onOpenPage: (page: string) => void }) {
  ...
  const activeProductionIds = scopedProductions.filter(item => !item.completed_at && !["failed", "cancelled"].includes(item.state)).map(item => item.id).join("|");
  useEffect(() => {
