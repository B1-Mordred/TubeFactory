...
type WorkflowActivityStatus = { activity_id: string; activity_type: string; label: string; state: string; attempt: number | null; maximum_attempts: number | null; scheduled_at: string | null; started_at: string | null; last_heartbeat_at: string | null; worker_identity: string | null; heartbeat: Record<string, string | number | boolean | null | undefined> };
type EditorialRun = { workflow_id: string; state: string; progress: number; result: Record<string, unknown> | null; execution_status: "RUNNING" | "COMPLETED" | "FAILED" | "CANCELLED" | "TERMINATED" | "TIMED_OUT"; retryable: boolean; correlation_id: string | null; activity_status?: WorkflowActivityStatus[] };
...
type StoryboardSummary = { id: string; script_id: string; status: string; version: number; current_version_id: string; content_hash: string; scene_count: number; placeholder_tokens: string[]; created_at: string };
...
type VoiceProfile = { id: string; profile_key: string; version_number: number; provider_type: string; language: string; engine: string; model_version: string; delivery?: Record<string, unknown>; output_settings?: Record<string, unknown>; pronunciation?: Record<string, unknown>; consent?: Record<string, unknown>; content_hash: string; enabled: boolean; active: boolean };
...
type TimelineScene = { id: string; role: "intro" | "storyboard" | "outro"; scene_version_id?: string | null; title: string; start_seconds: number; duration_seconds: number; end_seconds: number; on_screen_text?: string[]; narration_segment_ids?: string[]; video: { mode: string; asset_id: string | null; source_start_seconds: number; source_end_seconds: number | null; fit: string; volume: number }; can_delete?: boolean; locked_to_narration?: boolean; [key: string]: unknown };
type TimelineAudioClip = { id: string; kind: string; asset_id: string | null; start_seconds: number; duration_seconds: number; end_seconds: number; source_start_seconds: number; source_end_seconds: number | null; volume: number; locked?: boolean; [key: string]: unknown };
type MediaTimelinePlan = { schema_version: "media_timeline.v1"; production_id: string; storyboard_version_id: string; storyboard_hash: string; render_tier: string; width: number; height: number; fps: number; duration_seconds: number; narration: { asset_id: string; object_key: string; duration_seconds: number; chapters?: Array<Record<string, unknown>> }; voice_profile?: { profile_key?: string; engine?: string; language?: string; [key: string]: unknown }; scenes: TimelineScene[]; tracks: { video: Array<Record<string, unknown>>; audio: TimelineAudioClip[]; music?: TimelineAudioClip[]; sfx?: TimelineAudioClip[] }; content_hash?: string; updated_at?: string; [key: string]: unknown };
type MediaProduction = { id: string; storyboard_version_id: string; storyboard_hash: string; workflow_id: string; render_tier: string; state: string; settings: Record<string, unknown>; correlation_id: string; created_at: string; completed_at: string | null; assets: MediaAsset[]; render: { id: string; number: number; tier: string; video_asset_id: string; content_hash: string; engine: string; engine_version: string; composition: string; settings: Record<string, unknown>; probe: Record<string, unknown>; created_at: string } | null; manifest: { id: string; version: number; content_hash: string; object_key: string; document: { scenes?: ManifestScene[]; sources?: ManifestSource[]; chapters?: Array<{ title?: string; start_seconds?: number; timecode_seconds?: number }>; captions?: MediaAsset[]; narration?: Array<{ script_segment_id: string; segment_order: number; duration_seconds: number; request: { text?: string }; response: Record<string, unknown> }>; [key: string]: unknown } } | null; qa: { id: string; verdict: string; content_hash: string; metrics: Record<string, unknown>; policy_snapshot: Record<string, unknown> } | null; findings: QAFinding[]; approval: { id: string; decision: string; comment: string; actor_id: string; created_at: string } | null; timeline_plan?: MediaTimelinePlan | null; timeline_plan_hash?: string | null; automatic_continuation?: AutomaticContinuation | null };
...
async function api<T>(path: string, options: RequestInit = {}, csrf = ""): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new Error(formatApiErrorDetail(detail, response.statusText));
  }
...
function formatApiErrorDetail(detail: unknown, fallback: string): string {
  ...
  if (Array.isArray(detail)) {
    const messages = detail.map(item => {
      if (item && typeof item === "object") {
        ...
        const location = Array.isArray(record.loc) ? record.loc.map(String).join(".") : typeof record.loc === "string" ? record.loc : "";
        return location && message ? `${location}: ${message}` : message || JSON.stringify(record);
      }
      return String(item);
    }).filter(Boolean);
  ...
  if (detail && typeof detail === "object") {
    ...
    if (Array.isArray(record.placeholder_tokens) && record.placeholder_tokens.length) {
      parts.push(`Placeholders: ${record.placeholder_tokens.map(String).join(", ")}`);
    }
...
function humanizeState(value: unknown): string {
  return String(value || "")
    .replace(/^PENDING_ACTIVITY_STATE_/, "")
    .replaceAll("_", " ")
    .toLowerCase();
}
...
function WorkflowActivityDetails({ run }: { run: { activity_status?: WorkflowActivityStatus[] } }) {
  const activity = run.activity_status?.[0];
  if (!activity) return null;
  const heartbeat = activity.heartbeat || {};
  const provider = typeof heartbeat.provider === "string" ? heartbeat.provider.replaceAll("_", " ") : "";
  const heartbeatState = typeof heartbeat.state === "string" ? humanizeState(heartbeat.state) : "";
  const attempt = numeric(heartbeat.attempt) ?? activity.attempt;
  const maxAttempts = numeric(heartbeat.max_attempts) ?? activity.maximum_attempts;
  const segmentOrder = numeric(heartbeat.segment_order);
  const segmentCount = numeric(heartbeat.segment_count);
  const chunkIndex = numeric(heartbeat.chunk_index);
  const chunkCount = numeric(heartbeat.chunk_count);
  const statusCode = numeric(heartbeat.status_code);
  const delay = numeric(heartbeat.delay_seconds);
  const elapsed = numeric(heartbeat.elapsed_seconds);
  const detail = typeof heartbeat.detail === "string" ? heartbeat.detail : "";
  const lastHeartbeat = activity.last_heartbeat_at ? new Date(activity.last_heartbeat_at).toLocaleTimeString() : "";
  const bits = [
    provider,
    heartbeatState || humanizeState(activity.state),
    segmentOrder && segmentCount ? `segment ${segmentOrder}/${segmentCount}` : "",
    chunkIndex && chunkCount ? `chunk ${chunkIndex}/${chunkCount}` : "",
    attempt && maxAttempts ? `attempt ${attempt}/${maxAttempts}` : "",
    statusCode ? `HTTP ${statusCode}` : "",
    delay !== null ? `retry in ${Math.round(delay)}s` : "",
    elapsed !== null ? `${Math.round(elapsed)}s elapsed` : "",
  ].filter(Boolean);
  return <div className={`activity-status ${statusCode && statusCode >= 500 ? "waiting" : ""}`} role="status">
    <strong>{activity.label || activity.activity_type.replaceAll("-", " ")}</strong>
    <span>{bits.join(" · ")}</span>
    {detail && <small>{detail}</small>}
    {lastHeartbeat && <small>Last heartbeat {lastHeartbeat}{activity.worker_identity ? ` · worker ${activity.worker_identity}` : ""}</small>}
  </div>;
}
...
function cloneTimelinePlan(plan: MediaTimelinePlan | null | undefined): MediaTimelinePlan | null {
  return plan ? JSON.parse(JSON.stringify(plan)) as MediaTimelinePlan : null;
}
...
function clipLabel(asset: MediaAsset): string {
  const name = String(asset.generation_provenance.filename || asset.object_key.split("/").pop() || asset.id);
  const duration = asset.duration_seconds ? ` · ${secondsLabel(asset.duration_seconds)}` : "";
  const dimensions = asset.width && asset.height ? ` · ${asset.width}×${asset.height}` : "";
  return `${name}${duration}${dimensions}`;
}
...
function ResearchPanel({ csrf, role, activeChannelId }: { csrf: string; role: Role; activeChannelId: string }) {
  ...
  type ResearchWorkflowView = {
    ...
    activity_status?: WorkflowActivityStatus[];
  };
...
function EditorialPanel({ csrf, role, activeChannelId }: { csrf: string; role: Role; activeChannelId: string }) {
  ...
  const [storyboardScript, setStoryboardScript] = useState<ScriptDetail | null>(null);
  ...
  useEffect(() => { refresh(); }, [refresh]);
  useEffect(() => { setScript(null); setEdited(null); setDossier(null); setStoryboard(null); setStoryboardScript(null); }, [activeChannelId]);
  useEffect(() => {
    if (!run || run.execution_status !== "RUNNING") return;
    ...
    source.addEventListener("status", event => {
      try {
        ...
        if (next.execution_status !== "RUNNING") {
          ...
          if (storyboard) api<StoryboardDetail>(`/api/v1/editorial/storyboards/${storyboard.id}`).then(async value => { setStoryboard(value); setStoryboardScript(await api<ScriptDetail>(`/api/v1/editorial/scripts/${value.script_id}`)); });
  ...
  async function openStoryboard(id: string) {
    try {
      const detail = await api<StoryboardDetail>(`/api/v1/editorial/storyboards/${id}`);
      const linkedScript = await api<ScriptDetail>(`/api/v1/editorial/scripts/${detail.script_id}`);
      setStoryboard(detail); setStoryboardScript(linkedScript); setSelectedScene(null); setSceneAlternatives([]);
    }
  ...
  async function startStoryboardForScript(item: ScriptSummary) {
    try {
      const next = await api<EditorialRun>(`/api/v1/editorial/scripts/${item.id}/storyboard-runs`, { method: "POST", body: JSON.stringify({ script_version_id: item.current_version_id, sensitivity: "internal", idempotency_key: `ui-${randomUuid()}` }) }, csrf);
      setRun(next); setMessage(`Storyboard generation started for ${item.title}.`); await refresh();
    } catch (caught) { setMessage(caught instanceof Error ? caught.message : "Storyboard generation could not be started"); }
  }
  ...
  async function replacePlaceholders(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!storyboard || !storyboardScript) return;
    const data = new FormData(event.currentTarget);
    const replacements = Object.fromEntries(
      placeholderTokens
        .map((token, index) => [token, String(data.get(`placeholder_${index}`) || "").trim()])
        .filter(([, value]) => value)
    );
    try {
      const next = await api<StoryboardDetail>(`/api/v1/editorial/storyboards/${storyboard.id}/placeholders/replace`, { method: "POST", body: JSON.stringify({
        expected_script_version: storyboardScript.version,
        expected_script_hash: storyboardScript.content_hash,
        expected_storyboard_version: storyboard.version,
        expected_storyboard_hash: storyboard.content_hash,
        replacements,
        comment,
      }) }, csrf);
      const linkedScript = await api<ScriptDetail>(`/api/v1/editorial/scripts/${next.script_id}`);
      setStoryboard(next); setStoryboardScript(linkedScript); setScript(linkedScript); setEdited(structuredClone(linkedScript)); setMessage(next.automatic_continuation?.message || "Placeholder values saved as new immutable versions."); await refresh();
    } catch (caught) { setMessage(caught instanceof Error ? caught.message : "Placeholder replacement failed"); }
  }
  ...
  const placeholderTokens = Array.from(new Set([...(storyboard?.placeholder_tokens || []), ...(storyboardScript?.verification_report.unresolved_placeholders || storyboardScript?.verification_report.placeholder_tokens || [])])).sort();
  const storyboardScriptApproved = !storyboardScript || storyboardScript.status === "approved";
  ...
  return <><div className="page-heading"><div><p className="eyebrow">Increment 2 editorial production</p><h1>Scripts and storyboards</h1></div><span className="status good">Sentence-level evidence gate online</span></div>{message && <div className="notice">{message}</div>}{run && <section className="panel workflow-monitor"><div><span className={`status ${run.execution_status === "COMPLETED" ? "good" : "waiting"}`}>{run.state} · {run.progress}%</span><progress max="100" value={run.progress}>{run.progress}%</progress><code>{run.workflow_id}</code></div><WorkflowActivityDetails run={run} /></section>}
    ...
    <div className="research-grid"><section className="panel"><div className="panel-title"><div><p className="eyebrow">Versioned writing</p><h2>Scripts</h2></div><span className="chip">{scopedScripts.length}</span></div><div className="card-list">{scopedScripts.map(item => <button className="dossier-card" key={item.id} onClick={() => openScript(item.id)}><span><strong>{item.title}</strong><small>v{item.version} · {item.evidence_required ? `coverage ${item.coverage_percent}%` : "direct scripted video · no evidence check"} · {item.content_hash.slice(0, 12)}…</small></span><span className={`status ${item.status === "verified" || item.status === "approved" ? "good" : "waiting"}`}>{item.status}</span></button>)}{!scopedScripts.length && <p className="empty">No scripts belong to this channel yet.</p>}</div></section><section className="panel"><div className="panel-title"><div><p className="eyebrow">Strict SceneSpec</p><h2>Storyboards</h2></div><span className="chip">{scopedStoryboards.length}</span></div><div className="card-list">{scopedStoryboards.map(item => <button className="dossier-card" key={item.id} onClick={() => openStoryboard(item.id)}><span><strong>{scopedScripts.find(value => value.id === item.script_id)?.title || "Storyboard"}</strong><small>v{item.version} · {item.scene_count} scenes · {item.content_hash.slice(0, 12)}…{item.placeholder_tokens.length ? ` · ${item.placeholder_tokens.length} placeholder${item.placeholder_tokens.length === 1 ? "" : "s"}` : ""}</small>{item.placeholder_tokens.length > 0 && <small className="danger">Resolve before rendering: {item.placeholder_tokens.slice(0, 4).join(", ")}{item.placeholder_tokens.length > 4 ? "…" : ""}</small>}</span><span className={`status ${item.placeholder_tokens.length ? "bad" : item.status === "approved" ? "good" : "waiting"}`}>{item.placeholder_tokens.length ? "placeholders" : item.status}</span></button>)}{scopedScripts.filter(item => item.status === "approved" && !scopedStoryboards.some(value => value.script_id === item.id)).map(item => <div className="notice" key={item.id}><span>Storyboard generation for {item.title} should continue automatically after script approval.</span>{(role === "admin" || role === "operator") && <button type="button" className="secondary compact" onClick={() => startStoryboardForScript(item)}>Start / retry storyboard generation</button>}</div>)}{!scopedStoryboards.length && !scopedScripts.some(item => item.status === "approved") && <p className="empty">No storyboard is ready in this channel.</p>}</div></section></div>
    ...
    {storyboard && <section className="panel storyboard-editor">
      <div className="panel-title"><div><p className="eyebrow">Storyboard editor · immutable version {storyboard.version}</p><h2>{storyboard.scene_count} validated scenes</h2></div><div className="actions"><span className={`status ${placeholderTokens.length ? "bad" : storyboard.status === "approved" ? "good" : "waiting"}`}>{placeholderTokens.length ? "placeholders" : storyboard.status}</span><button className="secondary compact" onClick={() => { setStoryboard(null); setStoryboardScript(null); setSelectedScene(null); setSceneAlternatives([]); }}>Close</button></div></div>
      {placeholderTokens.length > 0 && <form className="readiness-card" onSubmit={replacePlaceholders}>
        <div className="panel-title"><div><strong>Provide placeholder values</strong><p className="muted">These values replace tokens in narration and storyboard scenes as new immutable versions. Review and approval are required again before rendering.</p></div><span className="status bad">{placeholderTokens.length} unresolved</span></div>
        <div className="form-pair">{placeholderTokens.map((token, index) => <label key={token}>{token}<input name={`placeholder_${index}`} placeholder={`Value for ${token}`} required /></label>)}</div>
        <div className="review-bar"><label>Replacement review note<input value={comment} onChange={event => setComment(event.target.value)} minLength={10} /></label><div><strong>Exact source versions</strong><p>Script {storyboardScript ? `v${storyboardScript.version} · ${storyboardScript.content_hash.slice(0, 12)}…` : "loading…"} · Storyboard v{storyboard.version} · {storyboard.content_hash.slice(0, 12)}…</p></div><button className="primary" disabled={!storyboardScript || !(role === "admin" || role === "editor")}>Save placeholder values</button></div>
      </form>}
      <div className="scene-grid">{storyboard.scenes.map(item => <article className={`scene-card ${item.locked ? "locked" : ""}`} key={item.id}><header><span className="chip">Scene {String(item.scene_spec.order)} · {String(item.scene_spec.visual_type)}</span><span>{Number(item.scene_spec.duration)}s</span></header><strong>{String(item.scene_spec.purpose)}</strong><p>{String(item.scene_spec.visual_brief)}</p><div className="claim-badges">{((item.scene_spec.claim_ids as string[] | undefined) || []).map(id => <span className="chip" key={id}>claim {id.slice(0, 8)}</span>)}</div><small>{item.content_hash.slice(0, 16)}… · scene v{item.version}</small>{(role === "admin" || role === "editor") && <div className="actions"><button className="secondary compact" onClick={() => setSceneLock(item, !item.locked)}>{item.locked ? "Unlock" : "Lock"}</button><button className="secondary compact" disabled={item.locked} onClick={() => openSceneEditor(item)}>Edit & alternatives</button><button className="secondary compact" disabled={item.locked} onClick={() => generateSceneAlternative(item)}>Regenerate</button></div>}</article>)}</div>
      {selectedScene && <div className="review-bar"><label>Strict SceneSpec JSON<textarea value={sceneJson} onChange={event => setSceneJson(event.target.value)} rows={18} /></label><div><strong>Generated alternatives</strong>{sceneAlternatives.length ? <div className="card-list">{sceneAlternatives.map(item => <article className="profile-card subject" key={item.id}><div><strong>Alternative {item.alternative_number}</strong><small>{String(item.scene_spec.visual_brief)} · {item.content_hash.slice(0, 12)}…</small><small>{item.current_base ? "Selectable for this scene version" : "Historical candidate"}</small></div><button className="secondary compact" disabled={!item.current_base} onClick={() => selectSceneAlternative(item)}>Select</button></article>)}</div> : <p className="empty">No generated alternatives for this scene version.</p>}</div><div className="actions"><button className="primary" onClick={saveScene}>Validate & create manual version</button><button className="secondary" onClick={() => { setSelectedScene(null); setSceneAlternatives([]); }}>Cancel</button></div></div>}
      <div className="review-bar"><label>Review / alternative instruction<input value={comment} onChange={event => setComment(event.target.value)} minLength={10} /></label><div><strong>{storyboardScriptApproved ? "Storyboard approval gate" : "Approve corrected script first"}</strong><p>{storyboardScriptApproved ? "The linked script hash is approved or unavailable in this view." : `Linked script is ${storyboardScript?.status}; approve its corrected hash before approving the storyboard.`}</p></div>{(role === "admin" || role === "reviewer") && storyboard.status === "in_review" && !placeholderTokens.length && <button className="primary" disabled={!storyboardScriptApproved} onClick={approveStoryboard}>{storyboardScriptApproved ? "Approve exact storyboard hash" : "Approve corrected script first"}</button>}</div>
    </section>}
  </>;
}
...
function MediaPanel({ csrf, role, activeChannelId, onOpenPage }: { csrf: string; role: Role; activeChannelId: string; onOpenPage: (page: string) => void }) {
  ...
  const [productionRun, setProductionRun] = useState<EditorialRun | null>(null);
  const [storyboardId, setStoryboardId] = useState("");
  const [workflowKey, setWorkflowKey] = useState("");
  const [voiceKey, setVoiceKey] = useState("");
  const [tier, setTier] = useState<"preview" | "full">("preview");
  const [mediaWorkflowType, setMediaWorkflowType] = useState<"narration_timeline" | "legacy_one_step">("narration_timeline");
  const [run, setRun] = useState<EditorialRun | null>(null);
  ...
  const [timelineDraft, setTimelineDraft] = useState<MediaTimelinePlan | null>(null);
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      setVoiceKey(current => {
        const currentVoice = nextVoices.find(item => item.profile_key === current && item.active && item.enabled && item.provider_type !== "fake");
        if (currentVoice) return currentVoice.profile_key;
        return nextVoices.find(item => item.active && item.enabled && item.provider_type !== "fake")?.profile_key || "";
      });
  ...
  useEffect(() => {
    if (!selected || ["timeline_ready", "ready", "blocked", "failed", "cancelled"].includes(selected.state)) {
      setProductionRun(null);
      return;
    }
    let cancelled = false;
    async function loadProductionRun() {
      if (!selected) return;
      try {
        const next = await api<EditorialRun>(`/api/v1/editorial/runs/${encodeURIComponent(selected.workflow_id)}`);
        if (!cancelled) setProductionRun(next);
      } catch {
        if (!cancelled) setProductionRun(null);
      }
    }
    loadProductionRun();
    const timer = window.setInterval(loadProductionRun, 5000);
    return () => { cancelled = true; window.clearInterval(timer); };
  }, [selected?.workflow_id, selected?.state]);
  useEffect(() => {
    setTimelineDraft(cloneTimelinePlan(selected?.timeline_plan));
    setAudioAssetId("");
  }, [selected?.id, selected?.timeline_plan_hash]);

  async function startProduction(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (mediaWorkflowType === "narration_timeline") {
      await startTimelineDraft();
      return;
    }
    ...
    try {
      ...
      setRun(next); setMessage(`Legacy one-step render started from exact storyboard hash ${storyboard.content_hash.slice(0, 12)}…. This path generates narration, visuals and render in one workflow.`);
    } catch (caught) { setMessage(caught instanceof Error ? caught.message : "Media production could not be started"); }
  }
  async function startTimelineDraft() {
    const storyboard = storyboards.find(item => item.current_version_id === storyboardId);
    if (!storyboard) return;
    try {
      const full = tier === "full";
      const next = await api<EditorialRun>("/api/v1/media/timeline-drafts", { method: "POST", body: JSON.stringify({ storyboard_version_id: storyboardId, expected_storyboard_hash: storyboard.content_hash, render_tier: tier, workflow_key: workflowKey, voice_profile_key: voiceKey, width: full ? 1920 : 854, height: full ? 1080 : 480, fps: full ? 24 : 12, idempotency_key: `ui-${randomUuid()}` }) }, csrf);
      setRun(next); setMessage(`Narration generation started. When it finishes, the measured audio will define the timeline editor.`);
    } catch (caught) { setMessage(caught instanceof Error ? caught.message : "Narration timeline could not be started"); }
  }
  async function uploadClip(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      body.set("file", clipFile);
      body.set("licence_basis", "operator_supplied");
      body.set("comment", "Uploaded for manual timeline editing.");
      await api<MediaAsset>(`/api/v1/media/productions/${selected.id}/clips`, { method: "POST", body }, csrf);
      setClipFile(null); setMessage("Clip uploaded and available in the timeline editor."); await refresh();
    } catch (caught) { setMessage(caught instanceof Error ? caught.message : "Clip upload failed"); }
  }
  function updateTimeline(mutator: (plan: MediaTimelinePlan) => void) {
    setTimelineDraft(current => {
      if (!current) return current;
      const next = cloneTimelinePlan(current);
      if (!next) return current;
      mutator(next);
      return next;
    });
  }
  function addBoundaryScene(role: "intro" | "outro") {
    updateTimeline(plan => {
      const scene: TimelineScene = { id: `${role}:${randomUuid()}`, role, title: role === "intro" ? "Intro" : "Outro", start_seconds: 0, duration_seconds: 3, end_seconds: 3, on_screen_text: [role === "intro" ? "Intro" : "Outro"], video: { mode: "generated", asset_id: null, source_start_seconds: 0, source_end_seconds: null, fit: "cover", volume: 0 }, can_delete: true, locked_to_narration: false };
      if (role === "intro") plan.scenes = [scene, ...plan.scenes];
      else plan.scenes = [...plan.scenes, scene];
    });
  }
  function updateScene(index: number, patch: Partial<TimelineScene>) {
    updateTimeline(plan => { plan.scenes[index] = { ...plan.scenes[index], ...patch }; });
  }
  function updateSceneVideo(index: number, patch: Partial<TimelineScene["video"]>) {
    updateTimeline(plan => { plan.scenes[index] = { ...plan.scenes[index], video: { ...plan.scenes[index].video, ...patch } }; });
  }
  function deleteScene(index: number) {
    updateTimeline(plan => { if (plan.scenes[index]?.role !== "storyboard") plan.scenes.splice(index, 1); });
  }
  function addAudioClip() {
    if (!audioAssetId || !selected) return;
    const asset = selected.assets.find(item => item.id === audioAssetId);
    if (!asset) return;
    updateTimeline(plan => {
      plan.tracks.audio = [...(plan.tracks.audio || []), { id: `audio:${randomUuid()}`, kind: "operator_audio", asset_id: audioAssetId, start_seconds: 0, duration_seconds: Math.min(asset.duration_seconds || 5, 30), end_seconds: Math.min(asset.duration_seconds || 5, 30), source_start_seconds: 0, source_end_seconds: null, volume: 0.35, locked: false }];
    });
    setAudioAssetId("");
  }
  function updateAudioClip(index: number, patch: Partial<TimelineAudioClip>) {
    updateTimeline(plan => { plan.tracks.audio[index] = { ...plan.tracks.audio[index], ...patch }; });
  }
  function deleteAudioClip(index: number) {
    updateTimeline(plan => { if (!plan.tracks.audio[index]?.locked) plan.tracks.audio.splice(index, 1); });
  }
  async function saveTimelinePlan(plan = timelineDraft) {
    if (!selected || !plan) return;
    try {
      const next = await api<MediaProduction>(`/api/v1/media/productions/${selected.id}/timeline-plan`, { method: "PUT", body: JSON.stringify({ expected_timeline_plan_hash: selected.timeline_plan_hash, timeline_plan: plan, comment: "Operator updated clip bindings and intro/outro timeline." }) }, csrf);
      setSelected(next); setMessage(`Timeline saved as ${next.timeline_plan_hash?.slice(0, 12)}….`); await refresh();
    } catch (caught) { setMessage(caught instanceof Error ? caught.message : "Timeline plan could not be saved"); }
  }
  async function queueTimelineRender() {
    if (!selected?.timeline_plan_hash) return;
    try {
      const next = await api<EditorialRun>(`/api/v1/media/productions/${selected.id}/timeline-render`, { method: "POST", body: JSON.stringify({ expected_timeline_plan_hash: selected.timeline_plan_hash, idempotency_key: `ui-${randomUuid()}` }) }, csrf);
      setRun(next); setMessage("Timeline render queued. TubeFactory will generate missing visuals, mix audio tracks and run media QA.");
    } catch (caught) { setMessage(caught instanceof Error ? caught.message : "Timeline render could not be queued"); }
  }
  ...
  const timelinePlan = timelineDraft;
  const operatorVideoClips = selected?.assets.filter(item => item.asset_kind === "operator_clip" && item.mime_type.startsWith("video/")) || [];
  const operatorImageClips = selected?.assets.filter(item => item.asset_kind === "operator_clip" && item.mime_type.startsWith("image/")) || [];
  const operatorVisualClips = [...operatorVideoClips, ...operatorImageClips];
  const operatorAudioClips = selected?.assets.filter(item => item.asset_kind === "operator_clip" && item.mime_type.startsWith("audio/")) || [];
  const narrationAssetId = timelinePlan?.narration?.asset_id || selected?.assets.find(item => item.asset_kind === "narration_mastered" && item.generation_provenance.assembly)?.id;
  const selectedVoice = voices.find(item => item.profile_key === voiceKey);
  ...
  const activeProductionIds = scopedProductions.filter(item => !item.completed_at && !["timeline_ready", "ready", "blocked", "failed", "cancelled"].includes(item.state)).map(item => item.id).join("|");
  useEffect(() => {
    if (!activeProductionIds) return;
    const timer = window.setInterval(() => { refresh(); }, 5000);
    return () => window.clearInterval(timer);
  }, [activeProductionIds, refresh]);
  const completedProductionCount = scopedProductions.filter(item => item.completed_at).length;
  ...
  const productionVoices = activeVoices.filter(item => item.provider_type !== "fake");
  const fixtureVoices = activeVoices.filter(item => item.provider_type === "fake");
  const selectedProductionVoice = productionVoices.find(item => item.profile_key === voiceKey);
  const selectedStoryboard = storyboards.find(item => item.current_version_id === storyboardId);
  const selectedPlaceholderTokens = selectedStoryboard?.placeholder_tokens || [];
  const renderBlockers = [
    ...
    productionVoices.length ? "" : "Register one enabled real Voicebox voice profile.",
    voiceKey && !selectedProductionVoice ? "Fixture voices are test tones; select a real Voicebox voice profile." : "",
    selectedPlaceholderTokens.length ? `Resolve storyboard placeholders before rendering: ${selectedPlaceholderTokens.join(", ")}` : "",
  ].filter(Boolean);
  const renderReady = renderBlockers.length === 0 && Boolean(storyboardId && workflowKey && selectedProductionVoice);
  const renderButtonLabel = !approvedStoryboards.length ? "Approve storyboard first" : !approvedWorkflows.length ? "Register workflow first" : !productionVoices.length ? "Register real voice first" : voiceKey && !selectedProductionVoice ? "Select real voice first" : !storyboardId ? "Select approved storyboard" : selectedPlaceholderTokens.length ? "Resolve placeholders first" : mediaWorkflowType === "legacy_one_step" ? "Start legacy one-step render" : "Start narration-first timeline";

  return <><div className="page-heading"><div><p className="eyebrow">Increment 3 media production</p><h1>Render and final review</h1></div><span className={`status ${scopedProductions.some(item => item.state === "ready") ? "good" : "waiting"}`}>{completedProductionCount} complete · {scopedProductions.length - completedProductionCount} active</span></div>
    {message && <div className="notice">{message}</div>}
    {run && <section className="panel workflow-monitor"><div><span className={`status ${run.execution_status === "COMPLETED" ? "good" : run.execution_status === "FAILED" ? "bad" : "waiting"}`}>{run.state} · {run.progress}%</span><progress max="100" value={run.progress}>{run.progress}%</progress><code>{run.workflow_id}</code></div><div><strong>{run.execution_status}</strong><p className="muted">Temporal retains this job across worker and host restarts.</p></div><button className="secondary compact" onClick={refresh}>Refresh</button><WorkflowActivityDetails run={run} /></section>}
    {canOperate && <section className="panel"><div className="panel-title"><div><p className="eyebrow">Exact approved input</p><h2>Start media workflow</h2></div><span className="chip">explicit workflow type</span></div><div className={`readiness-card ${renderBlockers.length ? "" : "ready"}`}><div className="panel-title"><div><strong>{renderBlockers.length ? "Rendering is waiting for prerequisites" : "Ready to start the selected workflow"}</strong><p className="muted">{renderBlockers.length ? "Fix the missing items below, then select the workflow type and start." : mediaWorkflowType === "legacy_one_step" ? "Legacy one-step render will immediately generate narration, visuals, final video and QA without timeline editing." : "Narration-first timeline generates audio first, then opens the timeline editor for clips, intro/outro and audio tracks."}</p></div><span className={`status ${renderBlockers.length ? "waiting" : "good"}`}>{renderBlockers.length ? `${renderBlockers.length} missing` : "ready"}</span></div><ul>{renderBlockers.length ? renderBlockers.map(item => <li key={item}>{item}</li>) : <li>Selected workflow type: {mediaWorkflowType === "legacy_one_step" ? "Legacy one-step render" : "Narration-first timeline"}.</li>}</ul><div className="actions">{(!approvedStoryboards.length || selectedPlaceholderTokens.length > 0) && <button type="button" className="secondary compact" onClick={() => onOpenPage("editorial")}>Open scripts & storyboards</button>}{role === "admin" && (!approvedWorkflows.length || !productionVoices.length) && <a className="secondary compact" href="#media-provider-registries">Open media registries</a>}</div></div><form className="media-start" onSubmit={startProduction}><label>Approved storyboard<select value={storyboardId} onChange={event => setStoryboardId(event.target.value)} required disabled={!approvedStoryboards.length}><option value="">Select exact version</option>{approvedStoryboards.map(item => <option key={item.current_version_id} value={item.current_version_id}>v{item.version} · {item.content_hash.slice(0, 12)}…{item.placeholder_tokens.length ? ` · ${item.placeholder_tokens.length} placeholder${item.placeholder_tokens.length === 1 ? "" : "s"}` : ""}</option>)}</select></label><label>Approved visual workflow<select value={workflowKey} onChange={event => setWorkflowKey(event.target.value)} required disabled={!approvedWorkflows.length}><option value="">Select workflow</option>{approvedWorkflows.map(item => <option key={item.id} value={item.workflow_key}>{item.workflow_key} v{item.version_number}</option>)}</select></label><label>Voice profile<select value={voiceKey} onChange={event => setVoiceKey(event.target.value)} required disabled={!productionVoices.length}><option value="">Select real Voicebox voice</option>{productionVoices.map(item => <option key={item.id} value={item.profile_key}>{item.profile_key} · {item.engine}</option>)}{role === "admin" && fixtureVoices.length ? <optgroup label="Fixture test voices (not renderable)">{fixtureVoices.map(item => <option key={item.id} value={item.profile_key} disabled>{item.profile_key} · TEST TONE</option>)}</optgroup> : null}</select></label><label>Tier<select value={tier} onChange={event => setTier(event.target.value as "preview" | "full")}><option value="preview">Low-resolution draft</option><option value="full">Full-resolution master</option></select></label><fieldset className="workflow-type-field"><legend>Workflow type</legend><label className={mediaWorkflowType === "narration_timeline" ? "selected" : ""}><input type="radio" name="media_workflow_type" value="narration_timeline" checked={mediaWorkflowType === "narration_timeline"} onChange={() => setMediaWorkflowType("narration_timeline")} /><span><strong>Narration-first timeline</strong><small>Generate narration now. Edit video/audio tracks, add intro/outro, then render the saved timeline.</small></span></label><label className={mediaWorkflowType === "legacy_one_step" ? "selected danger-choice" : "danger-choice"}><input type="radio" name="media_workflow_type" value="legacy_one_step" checked={mediaWorkflowType === "legacy_one_step"} onChange={() => setMediaWorkflowType("legacy_one_step")} /><span><strong>Legacy one-step render</strong><small>Immediately generate narration, visuals, render and QA. Use only for regression checks or old workflows.</small></span></label></fieldset><button className={mediaWorkflowType === "legacy_one_step" ? "secondary" : "primary"} disabled={!renderReady}>{renderButtonLabel}</button></form></section>}
    ...
    {selected && timelinePlan && <section className="panel timeline-editor"><div className="panel-title"><div><p className="eyebrow">Narration-first editor</p><h2>Timeline from mastered narration</h2><p className="muted">Storyboard scenes are locked to the measured narration. Add intro/outro scenes around them, attach clips, set trim points, save, then render.</p></div><span className="chip">{secondsLabel(timelinePlan.duration_seconds)} total</span></div><div className="timeline-status-strip"><div><strong>Narrator</strong><small>{selectedVoice ? `${selectedVoice.profile_key} · ${selectedVoice.engine} · ${selectedVoice.language}` : String(timelinePlan.voice_profile?.profile_key || "Saved voice profile")}</small></div><div><strong>Narration audio</strong><small>{secondsLabel(timelinePlan.narration.duration_seconds)} · {selected.timeline_plan_hash?.slice(0, 12)}…</small></div>{narrationAssetId && <audio controls preload="metadata" src={`/api/v1/media/assets/${narrationAssetId}/content`} />}</div>{canOperate && <form className="clip-upload" onSubmit={uploadClip}><label>Add media clip<input type="file" accept="image/png,image/jpeg,image/webp,video/mp4,video/webm,audio/*" onChange={event => setClipFile(event.target.files?.[0] || null)} /></label><button className="secondary" disabled={!clipFile}>Upload clip</button><small>Uploaded clips are immutable assets and can be reused by scene or audio tracks.</small></form>}<div className="timeline-actions"><button type="button" className="secondary compact" disabled={!canOperate} onClick={() => addBoundaryScene("intro")}>Add intro scene before narration</button><button type="button" className="secondary compact" disabled={!canOperate} onClick={() => addBoundaryScene("outro")}>Add outro scene after narration</button><button type="button" className="secondary compact" disabled={!canOperate} onClick={() => setTimelineDraft(cloneTimelinePlan(selected.timeline_plan))}>Discard local edits</button><button type="button" className="primary compact" disabled={!canOperate} onClick={() => saveTimelinePlan()}>Save timeline plan</button><button type="button" className="primary compact" disabled={!canOperate || !selected.timeline_plan_hash} onClick={queueTimelineRender}>2 · Render saved timeline</button></div><div className="track-editor"><section><h3>Video track</h3>{timelinePlan.scenes.map((scene, index) => <article className={`timeline-scene ${scene.role}`} key={scene.id}><header><span className={`status ${scene.role === "storyboard" ? "good" : "waiting"}`}>{scene.role}</span><strong>{secondsLabel(scene.start_seconds)}–{secondsLabel(scene.end_seconds)}</strong>{scene.role !== "storyboard" && <button type="button" className="secondary compact" onClick={() => deleteScene(index)}>Delete</button>}</header><div className="timeline-scene-grid"><label>Scene title<input value={scene.title || ""} onChange={event => updateScene(index, { title: event.target.value })} disabled={!canOperate || scene.role === "storyboard"} /></label><label>Duration seconds<input type="number" min="0.5" step="0.5" value={scene.duration_seconds} onChange={event => updateScene(index, { duration_seconds: Number(event.target.value) })} disabled={!canOperate || scene.role === "storyboard"} /></label><label>Video / image clip<select value={scene.video?.asset_id || ""} onChange={event => updateSceneVideo(index, { asset_id: event.target.value || null, mode: event.target.value ? "operator_clip" : "generated" })} disabled={!canOperate}><option value="">{scene.role === "storyboard" ? "Generate visual for this scene" : "Branded card / no clip"}</option>{operatorVisualClips.map(asset => <option key={asset.id} value={asset.id}>{clipLabel(asset)}</option>)}</select></label><label>Clip in<input type="number" min="0" step="0.1" value={scene.video?.source_start_seconds || 0} onChange={event => updateSceneVideo(index, { source_start_seconds: Number(event.target.value) })} disabled={!canOperate || !scene.video?.asset_id} /></label><label>Clip out<input type="number" min="0" step="0.1" value={scene.video?.source_end_seconds ?? ""} onChange={event => updateSceneVideo(index, { source_end_seconds: event.target.value ? Number(event.target.value) : null })} disabled={!canOperate || !scene.video?.asset_id} placeholder="auto" /></label><label>On-screen text<input value={(scene.on_screen_text || []).join(" / ")} onChange={event => updateScene(index, { on_screen_text: event.target.value.split("/").map(value => value.trim()).filter(Boolean) })} disabled={!canOperate || scene.role === "storyboard"} /></label></div></article>)}</section><section><h3>Audio tracks</h3><div className="audio-add-row"><label>Add audio bed / SFX<select value={audioAssetId} onChange={event => setAudioAssetId(event.target.value)} disabled={!operatorAudioClips.length}><option value="">Select uploaded audio</option>{operatorAudioClips.map(asset => <option key={asset.id} value={asset.id}>{clipLabel(asset)}</option>)}</select></label><button type="button" className="secondary compact" disabled={!audioAssetId} onClick={addAudioClip}>Add audio track</button></div>{timelinePlan.tracks.audio.map((clip, index) => <article className="audio-track-row" key={clip.id}><header><span className={`status ${clip.locked ? "good" : "waiting"}`}>{clip.kind}</span><strong>{clip.locked ? "Narration master" : operatorAudioClips.find(asset => asset.id === clip.asset_id)?.object_key.split("/").pop() || clip.asset_id}</strong>{!clip.locked && <button type="button" className="secondary compact" onClick={() => deleteAudioClip(index)}>Delete</button>}</header><div className="timeline-scene-grid"><label>Start<input type="number" min="0" step="0.1" value={clip.start_seconds} onChange={event => updateAudioClip(index, { start_seconds: Number(event.target.value) })} disabled={!canOperate || clip.locked} /></label><label>Duration<input type="number" min="0.05" step="0.1" value={clip.duration_seconds} onChange={event => updateAudioClip(index, { duration_seconds: Number(event.target.value) })} disabled={!canOperate || clip.locked} /></label><label>Source in<input type="number" min="0" step="0.1" value={clip.source_start_seconds} onChange={event => updateAudioClip(index, { source_start_seconds: Number(event.target.value) })} disabled={!canOperate || clip.locked} /></label><label>Source out<input type="number" min="0" step="0.1" value={clip.source_end_seconds ?? ""} onChange={event => updateAudioClip(index, { source_end_seconds: event.target.value ? Number(event.target.value) : null })} disabled={!canOperate || clip.locked} placeholder="auto" /></label><label>Volume<input type="number" min="0" max="2" step="0.05" value={clip.volume} onChange={event => updateAudioClip(index, { volume: Number(event.target.value) })} disabled={!canOperate || clip.locked} /></label></div></article>)}</section></div></section>}
    <div className="media-layout"><section className="panel"><div className="panel-title"><div><p className="eyebrow">Render queue and history</p><h2>Productions</h2></div><span className="chip">{scopedProductions.length}</span></div><div className="card-list">{scopedProductions.map(item => <button className="dossier-card" key={item.id} onClick={() => setSelected(item)}><span><strong>{item.render_tier === "full" ? "Full master" : item.state === "timeline_ready" ? "Narration timeline" : "Preview"} · {item.storyboard_hash.slice(0, 12)}…</strong><small>{new Date(item.created_at).toLocaleString()} · {item.render?.engine || item.timeline_plan_hash?.slice(0, 12) || "pending"}</small></span><span className={`status ${item.state === "ready" || item.state === "timeline_ready" ? "good" : item.state === "blocked" || item.state === "failed" ? "bad" : "waiting"}`}>{item.qa?.verdict || item.state}</span></button>)}{!scopedProductions.length && <p className="empty">No media production is queued or completed for this channel.</p>}</div></section>
    <section className="panel media-player"><div className="panel-title"><div><p className="eyebrow">Exact render review</p><h2>{selected ? `${selected.render_tier} · ${selected.state}` : "Select a production"}</h2></div>{selected?.approval && <span className={`status ${selected.approval.decision === "approved" ? "good" : "bad"}`}>{selected.approval.decision}</span>}</div>{videoId ? <video ref={videoRef} controls preload="metadata" key={videoId}><source src={`/api/v1/media/assets/${videoId}/content`} type="video/mp4" />{captionId && <track default kind="captions" srcLang="en" label="English" src={`/api/v1/media/assets/${captionId}/content`} />}</video> : <p className="empty">The render has not completed yet.</p>}{selected && productionRun && <div className="production-workflow-detail"><div><strong>{productionRun.execution_status}</strong><small>{productionRun.state} · {productionRun.progress}% · {productionRun.workflow_id}</small></div><WorkflowActivityDetails run={productionRun} /></div>}{selected?.render && <div className="hash-grid"><span>Render <code>{selected.render.content_hash}</code></span><span>Manifest <code>{selected.manifest?.content_hash}</code></span><span>Storyboard <code>{selected.storyboard_hash}</code></span></div>}</section></div>
...
function PublishingPanel({ csrf, role, activeChannelId, onOpenPage }: { csrf: string; role: Role; activeChannelId: string; onOpenPage: (page: string) => void }) {
  ...
  const existingUploadForSelection = scopedUploads.find(item => item.render_id === selectedRender && item.metadata_version_id === selectedMetadata && item.connection_id === connectionId && item.mode === mode);
  ...
  const uploadReadinessTitle = existingUploadForSelection ? `Private upload already ${existingUploadForSelection.state}` : uploadBlockers.length ? "Private upload is waiting" : "Ready for private upload approval";
  const uploadReadinessCopy = existingUploadForSelection ? `This exact render, metadata version, channel and ${mode} mode already have an upload entry. Use Upload history for reconciliation or public-release actions.` : uploadBlockers.length ? "A reviewer approves the exact metadata hash and render hash before any upload job is queued." : "Approve once for private upload; public release still requires a separate approval later.";
  const uploadReadinessStatus = existingUploadForSelection ? existingUploadForSelection.state : uploadBlockers.length ? `${uploadBlockers.length} missing` : "ready";
  const uploadActionLabel = existingUploadForSelection ? `Private upload ${existingUploadForSelection.state}` : selectedVersion && connectionId ? "Approve & queue private upload" : "Complete upload prerequisites";
  return <><div className="page-heading"><div><p className="eyebrow">Increment 4 publishing</p><h1>Private upload and release calendar</h1></div><span className={`status ${config?.real_uploads_enabled ? "waiting" : "good"}`}>{config?.real_uploads_enabled ? "real private upload enabled" : "dry-run only"}</span></div>
    ...
    <section className="panel">
      <div className="panel-title"><div><p className="eyebrow">Approval-bound upload</p><h2>Private first</h2></div><span className="chip">{connectedUploadTargets.length} active channel{connectedUploadTargets.length === 1 ? "" : "s"}</span></div>
      <div className={`readiness-card ${uploadBlockers.length ? "" : "ready"}`}>
        <div className="panel-title"><div><strong>{uploadReadinessTitle}</strong><p className="muted">{uploadReadinessCopy}</p></div><span className={`status ${uploadBlockers.length ? "waiting" : "good"}`}>{uploadReadinessStatus}</span></div>
        <ul>{existingUploadForSelection ? <li>Existing upload entry: {existingUploadForSelection.mode} · {existingUploadForSelection.state} · {existingUploadForSelection.youtube_video_id || "awaiting provider video ID"}.</li> : uploadBlockers.length ? uploadBlockers.map(item => <li key={item}>{item}</li>) : <li>Confirm the selected connection and execution mode, then approve the private upload queue entry.</li>}</ul>
      </div>
      <label>Metadata version<select value={selectedMetadata} onChange={event => setSelectedMetadata(event.target.value)} disabled={!scopedMetadata.some(item => item.render_id === selectedRender)}><option value="">Select metadata</option>{scopedMetadata.filter(item => item.render_id === selectedRender).map(item => <option key={item.id} value={item.id}>v{item.version_number} · {item.content_hash.slice(0, 12)}…</option>)}</select></label>
      <label>YouTube channel<select value={connectionId} onChange={event => setConnectionId(event.target.value)} disabled={!connectedUploadTargets.length}><option value="">Select channel</option>{connectedUploadTargets.map(item => <option key={item.id} value={item.id}>{item.youtube_channel_title}</option>)}</select></label>
      <label>Upload execution<select value={mode} onChange={event => setMode(event.target.value as "dry_run" | "real")}><option value="dry_run">Dry-run / mock provider</option><option value="real" disabled={!config?.real_uploads_enabled}>Real resumable private upload</option></select></label>
      {selectedVersion && <div className="hash-grid"><span>Render <code>{production?.render?.content_hash}</code></span><span>Metadata <code>{selectedVersion.content_hash}</code></span></div>}
      <div className="actions">{canReview && <button className="primary" disabled={!selectedVersion || !connectionId || Boolean(existingUploadForSelection)} onClick={() => approve("private_upload")}>{uploadActionLabel}</button>}</div>
    </section></div>
