  ...
  async function createSubject(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await api("/api/v1/subject-profiles", { method: "POST", body: JSON.stringify({
        ...
        related_concepts: [], negative_keywords: [], languages: [data.get("language") || "en"], regions: [],
        ...
        schedule: { cron: String(data.get("cron") || "").trim() || null, timezone: String(data.get("timezone") || "UTC").trim() }, freshness_policy: { lookback_days: 30, maximum_source_age_days: 3650 },
  ...
  return <><div className="page-heading"><div><p className="eyebrow">Discovery configuration</p><h1>Channels and subjects</h1></div><span className="status good">Monitoring online</span></div>
    ...
    <div className="profile-grid">
      <section className="panel"><div className="panel-title"><div><p className="eyebrow">Editorial identities</p><h2>Channel profiles</h2></div><span className="chip">{channels.length}</span></div>
        <div className="card-list">{channels.map(channel => <article className="profile-card" key={channel.id}><div><strong>{channel.name}</strong><small>{channel.languages.join(" · ")} · v{channel.version}</small></div><span className={`status ${channel.enabled ? "good" : ""}`}>{channel.enabled ? "Enabled" : "Disabled"}</span></article>)}{!channels.length && <p className="empty">Create a disabled editorial identity to begin.</p>}</div>
...
export default function Home() {
  ...
  const navGroups = [
    { label: "Workspace", items: [{ id: "dashboard", label: "Overview" }, { id: "workflows", label: "Workflow activity", roles: ["admin", "operator"] }] },
    { label: "Create and review", items: [{ id: "profiles", label: "Channels & subjects" }, { id: "research", label: "Research & evidence" }, { id: "editorial", label: "Scripts & storyboards" }, { id: "media", label: "Media & final review" }, { id: "publishing", label: "Publishing & calendar" }] },
    { label: "Operations", items: [{ id: "operations", label: "Analytics & operations" }, { id: "audit", label: "Audit log" }] },
    { label: "Administration", items: [{ id: "providers", label: "Providers & prompts", roles: ["admin"] }, { id: "configuration", label: "Configuration", roles: ["admin"] }, { id: "users", label: "Users", roles: ["admin"] }, { id: "identity", label: "Authentication" }] },
  ];
