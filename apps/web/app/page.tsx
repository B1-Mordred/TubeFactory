...
function MediaPanel({ csrf, role, activeChannelId, onOpenPage }: { csrf: string; role: Role; activeChannelId: string; onOpenPage: (page: string) => void }) {
  ...
  const renderButtonLabel = !approvedStoryboards.length ? "Approve storyboard first" : !approvedWorkflows.length ? "Register workflow first" : !activeVoices.length ? "Register voice first" : !storyboardId ? "Select approved storyboard" : selectedPlaceholderTokens.length ? "Resolve placeholders first" : "Queue durable render";
  ...
  return <><div className="page-heading"><div><p className="eyebrow">Increment 3 media production</p><h1>Render and final review</h1></div><span className={`status ${scopedProductions.some(item => item.state === "ready") ? "good" : "waiting"}`}>{completedProductionCount} complete · {scopedProductions.length - completedProductionCount} active</span></div>
    ...
    {canOperate && <section className="panel"><div className="panel-title"><div><p className="eyebrow">Exact approved input</p><h2>Create narration timeline</h2></div><span className="chip">narration first</span></div><div className={`readiness-card ${renderBlockers.length ? "" : "ready"}`}><div className="panel-title"><div><strong>{renderBlockers.length ? "Rendering is waiting for prerequisites" : "Ready to create a narrated timeline"}</strong><p className="muted">{renderBlockers.length ? "Fix the missing items below, then create narration first, edit the timeline, then render." : "The selected storyboard, visual workflow and voice profile are approved for this workspace."}</p></div><span className={`status ${renderBlockers.length ? "waiting" : "good"}`}>{renderBlockers.length ? `${renderBlockers.length} missing` : "ready"}</span></div><ul>{renderBlockers.length ? renderBlockers.map(item => <li key={item}>{item}</li>) : <li>Create the narration timeline first. Then attach clips, save the timeline, render, inspect QA and approve exact hashes.</li>}</ul><div className="actions">{(!approvedStoryboards.length || selectedPlaceholderTokens.length > 0) && <button type="button" className="secondary compact" onClick={() => onOpenPage("editorial")}>Open scripts & storyboards</button>}{role === "admin" && (!approvedWorkflows.length || !activeVoices.length) && <a className="secondary compact" href="#media-provider-registries">Open media registries</a>}</div></div><form className="media-start" onSubmit={startProduction}><label>Approved storyboard<select value={storyboardId} onChange={event => setStoryboardId(event.target.value)} required disabled={!approvedStoryboards.length}><option value="">Select exact version</option>{approvedStoryboards.map(item => <option key={item.current_version_id} value={item.current_version_id}>v{item.version} · {item.content_hash.slice(0, 12)}…{item.placeholder_tokens.length ? ` · ${item.placeholder_tokens.length} placeholder${item.placeholder_tokens.length === 1 ? "" : "s"}` : ""}</option>)}</select></label><label>Approved visual workflow<select value={workflowKey} onChange={event => setWorkflowKey(event.target.value)} required disabled={!approvedWorkflows.length}><option value="">Select workflow</option>{approvedWorkflows.map(item => <option key={item.id} value={item.workflow_key}>{item.workflow_key} v{item.version_number}</option>)}</select></label><label>Voice profile<select value={voiceKey} onChange={event => setVoiceKey(event.target.value)} required disabled={!activeVoices.length}><option value="">Select voice</option>{activeVoices.map(item => <option key={item.id} value={item.profile_key}>{item.profile_key} · {item.engine}</option>)}</select></label><label>Tier<select value={tier} onChange={event => setTier(event.target.value as "preview" | "full")}><option value="preview">Low-resolution draft</option><option value="full">Full-resolution master</option></select></label><button type="button" className="primary" disabled={!renderReady} onClick={startTimelineDraft}>1 · Generate narration timeline</button><button className="secondary" disabled={!renderReady} title="Legacy path: generate narration, visuals and render in one workflow">Legacy one-step render</button></form></section>}
